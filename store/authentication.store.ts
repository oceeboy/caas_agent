...
interface AuthenticatedUser extends UserTypes.User {}
/**
...
export type AuthenticationState = {
  ...
  userHasRole: (role: AuthenticatedUser['role']) => boolean;
    ...
    (set, get) => {
      ...
      return {
        ...
        isSessionValid: () => {
          ...
          return currentTime < accessTokenExpiresAt;
        },
        userHasRole: (role) => {
        ...
        refreshSessionIfNeeded: async () => {
          ...
          const { refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt, clearSession } = get();
          ...
          if (refreshTokenExpiresAt !== null && currentTime > refreshTokenExpiresAt) {
            clearSession();
            return;
          }
          // check if access token is valid for at least another minute
          if (!refreshToken || !refreshTokenExpiresAt || !accessTokenExpiresAt) {
            // prerequisites missing, nothing to do
            return;
          }
          // Refresh  60 seconds before expiry
          const shouldRefresh = currentTime >= accessTokenExpiresAt - 60000;
          if (!shouldRefresh) {
            return;
          }
